...
client.on('message', message => {
  ...
  if(message.channel.id == 793684121511526403) return message.reply("Please use these commands in <#793684851021578241>!")
	if (!cmd || (cmd.category == "Dev" && !config.staff.includes(message.author.id))) return message.reply('No such command found for ' + Discord.Util.cleanContent(config.prefix + args[0].toLowerCase(), message))
  if(cmd.category == "Fun") {
  ...
  try {
    ...
    message.reply(`Error: \`${err.toString()}\`\n**If you believe this is a mistake please contact report this error on the support server which you can get by doing ",invite"**`)
    console.error(err)
  }
})
